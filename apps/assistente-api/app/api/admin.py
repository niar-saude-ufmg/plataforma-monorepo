...
async def create_user(
    ...
    role_value = (body.role or "researcher").strip().lower()
    if role_value not in ("admin", "researcher"):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Papel inválido")
    role = UserRole.admin if role_value == "admin" else UserRole.researcher
    ...
    await log_audit(
        ...
        details=f"Created {user.email} as {role.value}",
    )
    await db.commit()
