...
def _messages_snapshot(session: WizardSession) -> str:
    return json.dumps(
        [{"role": m.role, "content": m.content, "created_at": m.created_at.isoformat()} for m in sorted(session.messages, key=lambda msg: msg.id)]
    )
    ...
    if body.linked_project_id is not None:
        project_result = await db.execute(
            select(WizardSession).where(
                WizardSession.id == body.linked_project_id,
                WizardSession.user_id == current_user.id,
        ...
        if not project_result.scalar_one_or_none():
            raise HTTPException(
                status_code=400,
                detail="linked_project_id deve apontar para um projeto do mesmo usuário",
            )
...
@router.post("/{session_id}/initial-script")
async def generate_initial_script(
    ...
    if not session:
        raise HTTPException(status_code=404, detail="Sessão não encontrada")
    if session.wizard_type != WizardType.data_clean:
        raise HTTPException(status_code=400, detail="Esta sessão não é de engenharia de dados")
    if session.script_content.strip():
        return {
            "script_content": session.script_content,
            "validation_result": json.loads(session.validation_result or "{}"),
            "already_exists": True,
    ...
    try:
        script = await generate_initial_clean_script(db, session)
