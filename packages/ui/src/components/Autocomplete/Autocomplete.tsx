...
export function Autocomplete({ options, label = "Selecionar", onChange, ...props }: AutocompleteProps) {
  ...
  return <MuiAutocomplete {...props} options={[...options]} getOptionLabel={getLabel} isOptionEqualToValue={(option, value) => (typeof option === "string" ? option : option.value) === (typeof value === "string" ? value : value.value)} onChange={(event, value, reason, details) => onChange?.(event, value, reason, details)} renderInput={(params) => <TextField {...params} label={label} />} />;
}
