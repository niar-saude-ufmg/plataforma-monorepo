...
it('alterna a senha preservando valor e foco sem submeter', () => {
  ...
  render(<form onSubmit={submit}><Input label="Senha" type="password" defaultValue="segredo" /></form>);
...
it('associa a mensagem de erro ao email', () => {
  render(<Input label="E-mail" type="email" error helperText="E-mail inválido" />);
...
it('seleciona uma opção e informa a alteração', () => {
  const change = vi.fn();
  render(<Select label="Base" defaultValue="own" onChange={change} options={[
    { value: 'own', label: 'Própria' }, { value: 'niar', label: 'NIAR' },
  ]} />);
  fireEvent.change(screen.getByRole('combobox', { name: 'Base' }), { target: { value: 'niar' } });
